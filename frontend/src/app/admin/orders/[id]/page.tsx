...
export default function AdminOrderDetailPage() {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      ...
      setIsLoading(true);
      try {
        const response = await fetch(API_ENDPOINTS.ORDERS.ADMIN.GET(orderId), {
  ...
  const updateDeliveryStatus = async (status: 'received' | 'processing' | 'delivered', message?: string) => {
    ...
    try {
      ...
      const response = await fetch(API_ENDPOINTS.ORDERS.ADMIN.UPDATE_DELIVERY_STATUS(order.id), {
        method: 'PUT',
        headers: getAuthHeaders(),
      ...
      if (responseData.status === 1) {
        setOrder({ ...order, deliveryStatus: status, adminMessage: message || order.adminMessage });
        if (message !== undefined) {
  ...
  if (authLoading || isLoading) {
    return (
      <div className={styles.container}>
        <PageHeader title="جزئیات سفارش" onBack={() => router.push("/admin/orders")} />
    ...
    <div className={styles.container}>
      ...
      <div className={styles.content}>
        ...
        <div className={styles.infoSection}>
          ...
          <div className={styles.fieldGroup}>
            <div className={styles.fieldContainer}>
              <div className={styles.fieldWrapper}>
                <label className={styles.fieldLabel}>نام کاربری:</label>
                <div className={styles.fieldItem}>
                  <span className={styles.fieldItemValue}>{order.userName || 'نامشخص'}</span>
                </div>
              </div>
              {order.phoneNumber && (
                <div className={styles.fieldWrapper}>
                  ...
                  <div className={styles.fieldItem}>
                    <span className={styles.fieldItemValue}>{order.phoneNumber}</span>
                  </div>
                </div>
              )}
            </div>
          ...
          {/* وضعیت تحویل */}
          <div className={styles.fieldGroup}>
            <h3 className={styles.sectionTitle}>وضعیت تحویل</h3>
            <div className={styles.statusButtons}>
              <button
                className={`${styles.statusButton} ${order.deliveryStatus === 'received' ? styles.activeStatus : ''}`}
                onClick={() => updateDeliveryStatus('received')}
                disabled={isUpdating || order.deliveryStatus === 'received'}
              >
                دریافت شده
              </button>
              <button
                className={`${styles.statusButton} ${order.deliveryStatus === 'processing' ? styles.activeStatus : ''}`}
                onClick={() => updateDeliveryStatus('processing')}
                disabled={isUpdating || order.deliveryStatus === 'processing'}
              >
                در حال پردازش
              </button>
            </div>
          ...
          {/* پیام ادمین */}
          <div className={styles.fieldGroup}>
            <label className={styles.fieldLabel}>پیام ادمین:</label>
            <textarea
              value={adminMessage}
              onChange={(e) => setAdminMessage(e.target.value)}
              onBlur={() => updateDeliveryStatus(order.deliveryStatus, adminMessage)}
              placeholder="پیام ادمین برای سفارش..."
              className={styles.adminMessageInput}
              rows={4}
            />
          </div>

          {/* دکمه تغییر وضعیت به تحویل شده */}
          {order.deliveryStatus !== 'delivered' && (
            <div className={styles.fieldGroup}>
              <button
                className={styles.deliveredButton}
                onClick={() => updateDeliveryStatus('delivered', adminMessage)}
                disabled={isUpdating}
              >
